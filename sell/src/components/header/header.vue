...
<script>
  ...
  export default {
  	...
  	methods: {	//methods将被混入到Vue实例中。可以直接通过 VM 实例访问这些方法
  		showDetail() {
...
<style lang="less" rel="stylesheet/less">
	...
	.header{
		...
		.detail{
			...
			height: 100%;
			overflow: auto;
			backdrop-filter: blur(10px);	//高斯模糊ios专用
			opacity: 1;
			background: rgba(7,17,27,0.8);
			&.fade-enter-active,&.fade-leave-active{
				transition: all 0.5s;	// vue过渡
			}
			&.fade-enter,&.fade-leave-active{
				opacity: 0;
				background: rgba(7,17,27,0);
			}
			.detail-Wrapper{		// Css sticky footer  布局
				width: 100%;
				min-height: 100%;
				.detail-main{
					margin-top: 64px;
